...
export default function Tags(props: { toggleChipProperty: (arg0: any) => void; }) {
  const classes = useStyles();
  const [chipData, setChipData] = useState<ChipData[]>([]);
  const [state, setstate] = useState<ChipData[]>([]);

  const addChip = (chip: any): void => {
    props.toggleChipProperty(chip);
  };
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            {...params}
            variant="filled"
            label="Choose your favourites tags ..."
          />
        )}
      />
      {/* <Autocomplete
